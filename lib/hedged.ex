defmodule Hedged do
  ...
  ## Quick start
      ...
      {:ok, body} = Hedged.run(fn -> fetch(url) end, delay: 100)
  ...
  ## Examples
      ...
      iex> Hedged.run(fn -> {:ok, 42} end)
      {:ok, 42}

      iex> Hedged.run(fn -> :hello end)
      {:ok, :hello}

      iex> Hedged.run(fn -> {:error, :boom} end, max_requests: 1)
      {:error, :boom}
  ...
  def run(fun, opts) when is_function(fun, 0) and is_list(opts) do
    ...
    case Hedged.Runner.execute(fun, runner_opts) do
  ...
  @doc """
  Executes `fun` with adaptive hedging controlled by a `Hedged.Tracker`.
  ...
  def run(server, fun, opts) when is_function(fun, 0) and is_list(opts) do
    {delay, allow_hedge?} = Hedged.Tracker.get_config(server)
    now_fn = Keyword.get(opts, :now_fn, &System.monotonic_time/1)
  ...
  @doc """
  Starts a `Hedged.Tracker` process linked to the current process.
  ...
  def start_link(opts) do
    Hedged.Tracker.start_link(opts)
  end
