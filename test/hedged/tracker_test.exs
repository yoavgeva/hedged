defmodule Hedged.TrackerTest do
  ...
  describe "get_config/1" do
    test "returns initial_delay before min_samples reached" do
      name = :"tracker_#{System.unique_integer([:positive])}"
      start_supervised!({Hedged.Tracker, name: name, initial_delay: 200, min_samples: 5})

      assert {200, true} = Hedged.Tracker.get_config(name)
    end
    ...
    test "adapts delay after enough samples" do
      ...
      start_supervised!(
        {Hedged.Tracker, name: name, min_samples: 5, percentile: 95, initial_delay: 200}
      )

      # Record 20 samples with latencies 10..30
      for i <- 10..30 do
        Hedged.Tracker.record(name, %{latency_ms: i, hedged?: false, hedge_won?: false})
      end

      # Allow cast to process
      _ = Hedged.Tracker.stats(name)

      {delay, _allow?} = Hedged.Tracker.get_config(name)
    ...
    test "clamps delay to min_delay" do
      ...
      start_supervised!(
        {Hedged.Tracker, name: name, min_samples: 2, min_delay: 50, percentile: 50}
      )

      for _ <- 1..5 do
        Hedged.Tracker.record(name, %{latency_ms: 1, hedged?: false, hedge_won?: false})
      end

      _ = Hedged.Tracker.stats(name)
      {delay, _} = Hedged.Tracker.get_config(name)
      assert delay >= 50
    end
    ...
    test "clamps delay to max_delay" do
      ...
      start_supervised!(
        {Hedged.Tracker, name: name, min_samples: 2, max_delay: 100, percentile: 99}
      )

      for _ <- 1..10 do
        Hedged.Tracker.record(name, %{latency_ms: 5000, hedged?: false, hedge_won?: false})
      end
  ...
  describe "token bucket" do
    test "tokens deplete with hedging, disabling further hedges" do
      ...
      Hedged.Tracker.record(name, %{latency_ms: 10, hedged?: true, hedge_won?: false})
      _ = Hedged.Tracker.stats(name)
      {_, allow?} = Hedged.Tracker.get_config(name)
      assert allow? == true
    ...
    test "tokens replenish with non-hedged requests" do
      ...
      Hedged.Tracker.record(name, %{latency_ms: 10, hedged?: false, hedge_won?: false})
    ...
    test "tokens do not exceed max" do
      ...
      Hedged.Tracker.record(name, %{latency_ms: 10, hedged?: false, hedge_won?: false})
      stats = Hedged.Tracker.stats(name)
  ...
  describe "stats/1" do
    test "tracks counters correctly" do
      name = :"tracker_#{System.unique_integer([:positive])}"
      start_supervised!({Hedged.Tracker, name: name})

      Hedged.Tracker.record(name, %{latency_ms: 10, hedged?: false, hedge_won?: false})
      Hedged.Tracker.record(name, %{latency_ms: 20, hedged?: true, hedge_won?: false})
      Hedged.Tracker.record(name, %{latency_ms: 15, hedged?: true, hedge_won?: true})
    ...
    test "concurrent get_config and record don't deadlock" do
      ...
      tasks =
        for i <- 1..100 do
          Task.async(fn ->
            if rem(i, 2) == 0 do
              Hedged.Tracker.get_config(name)
            else
              Hedged.Tracker.record(name, %{
                latency_ms: i,
                hedged?: false,
  ...
  describe "percentile edge cases" do
    test "all identical latencies" do
      ...
      start_supervised!(
        {Hedged.Tracker, name: name, min_samples: 2, min_delay: 1, max_delay: 5_000}
      )

      for _ <- 1..20 do
        Hedged.Tracker.record(name, %{latency_ms: 50, hedged?: false, hedge_won?: false})
      end
    ...
    test "bimodal distribution" do
      ...
      start_supervised!({Hedged.Tracker, name: name, min_samples: 2, percentile: 50})
      ...
      for _ <- 1..50 do
        Hedged.Tracker.record(name, %{latency_ms: 1000, hedged?: false, hedge_won?: false})
      end
    ...
    test "delay adapts when latency pattern changes" do
      ...
      _ = Hedged.Tracker.stats(name)
      {delay_fast, _} = Hedged.Tracker.get_config(name)

      # Phase 2: slow latencies (evicts old fast ones)
      for _ <- 1..20 do
        Hedged.Tracker.record(name, %{latency_ms: 500, hedged?: false, hedge_won?: false})
      end

      _ = Hedged.Tracker.stats(name)
      {delay_slow, _} = Hedged.Tracker.get_config(name)
    ...
    test "min_samples: 0 adapts from the first sample" do
      ...
      {delay_empty, _} = Hedged.Tracker.get_config(name)
      assert delay_empty == 500

      Hedged.Tracker.record(name, %{latency_ms: 20, hedged?: false, hedge_won?: false})
      _ = Hedged.Tracker.stats(name)
      {delay_one, _} = Hedged.Tracker.get_config(name)
      assert delay_one == 20
    end
  end
  ...
  describe "stats (extended)" do
    test "stats includes current_delay and tokens" do
      name = :"tracker_#{System.unique_integer([:positive])}"
      start_supervised!({Hedged.Tracker, name: name, initial_delay: 200, token_max: 10})
    ...
    test "tracker remains functional after recording errors" do
      ...
      for _ <- 1..5 do
        Hedged.Tracker.record(name, %{latency_ms: 5000, hedged?: true, hedge_won?: false})
      end
      ...
      {_delay, _allow?} = Hedged.Tracker.get_config(name)
    end

    test "buffer overflow with rapid records" do
      name = :"tracker_#{System.unique_integer([:positive])}"
      start_supervised!({Hedged.Tracker, name: name, buffer_size: 10})
