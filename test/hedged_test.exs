defmodule HedgedTest do
  ...
  describe "run/2 (stateless)" do
    test "returns {:ok, value} on success" do
      assert {:ok, 42} = Hedged.run(fn -> {:ok, 42} end)
    end

    test "returns {:error, reason} on failure" do
      assert {:error, :boom} = Hedged.run(fn -> {:error, :boom} end, max_requests: 1)
    end

    test "normalizes bare values" do
      assert {:ok, :hello} = Hedged.run(fn -> :hello end)
    end
    ...
    test "hedge wins over slow first request" do
      ...
      assert {:ok, :fast} = Hedged.run(fun, delay: 10)
    end
    ...
    test "respects timeout" do
      ...
      assert {:error, :timeout} = Hedged.run(fun, timeout: 50, delay: 10)
    end

    test "default options work" do
      assert {:ok, :works} = Hedged.run(fn -> {:ok, :works} end)
    end
    ...
    test "max_requests: 1 disables hedging" do
      ...
      assert {:ok, :done} = Hedged.run(fun, max_requests: 1)
  ...
  describe "run/3 (stateful with tracker)" do
    ...
    test "returns {:ok, value} on success", %{tracker: tracker} do
      assert {:ok, 42} = Hedged.run(tracker, fn -> {:ok, 42} end, [])
    end

    test "returns {:error, reason} on failure", %{tracker: tracker} do
      assert {:error, :boom} =
               Hedged.run(tracker, fn -> {:error, :boom} end, max_requests: 1)
    end

    test "records latency in tracker stats", %{tracker: tracker} do
      Hedged.run(tracker, fn -> {:ok, :done} end, [])
      stats = Hedged.Tracker.stats(tracker)
    ...
    test "records hedged requests in stats", %{tracker: tracker} do
      ...
      Hedged.run(tracker, fun, timeout: 5_000)
      stats = Hedged.Tracker.stats(tracker)
    ...
    test "adaptive delay changes with latency data" do
      ...
      {delay_before, _} = Hedged.Tracker.get_config(name)
      ...
      {delay_after, _} = Hedged.Tracker.get_config(name)
  ...
  describe "run/3 (extended)" do
    ...
    test "records hedge_won when hedge beats first request", %{tracker: tracker} do
      ...
      assert {:ok, :fast} = Hedged.run(tracker, fun, [])
    ...
    test "tracker continues working after all-fail scenario", %{tracker: tracker} do
      # All-fail run
      Hedged.run(tracker, fn -> {:error, :nope} end, max_requests: 1)

      # Normal run after
      assert {:ok, :fine} = Hedged.run(tracker, fn -> {:ok, :fine} end, [])
    ...
    test "100 concurrent stateful runs", %{tracker: tracker} do
      tasks =
        for i <- 1..100 do
          Task.async(fn ->
            Hedged.run(tracker, fn -> {:ok, i} end, [])
          end)
        end
    ...
    test "timeout works through stateful API", %{tracker: tracker} do
      ...
      assert {:error, :timeout} = Hedged.run(tracker, fun, timeout: 50)
    end
  end
  ...
  describe "start_link/1 and child_spec/1" do
    test "start_link starts a tracker" do
      ...
      pid = start_supervised!({Hedged, name: name})
      assert is_pid(pid)
      assert Hedged.Tracker.stats(name).total_requests == 0
    end

    test "child_spec uses name as id" do
      spec = Hedged.child_spec(name: MyApp.Hedged)
      assert spec.id == MyApp.Hedged
    end

    test "child_spec defaults id to Hedged when no name" do
      spec = Hedged.child_spec([])
      assert spec.id == Hedged
    end
    ...
    test "works under a supervisor" do
      ...
      assert {:ok, 42} = Hedged.run(name, fn -> {:ok, 42} end, [])
  ...
  describe "run/2 (extended edge cases)" do
    test "nested {:ok, {:error, _}} is treated as success" do
      assert {:ok, {:error, :inner}} = Hedged.run(fn -> {:ok, {:error, :inner}} end)
    end

    test "bare :error returns {:error, :error}" do
      assert {:error, :error} = Hedged.run(fn -> :error end, max_requests: 1)
    end

    test "bare :ok returns {:ok, :ok}" do
      assert {:ok, :ok} = Hedged.run(fn -> :ok end)
    end

    test "bare nil returns {:ok, nil}" do
      assert {:ok, nil} = Hedged.run(fn -> nil end)
    end

    test "bare map returns {:ok, map}" do
      assert {:ok, %{key: :value}} = Hedged.run(fn -> %{key: :value} end)
    end
    ...
    test "100 concurrent stateless runs" do
      tasks =
        for i <- 1..100 do
          Task.async(fn ->
            Hedged.run(fn -> {:ok, i} end, delay: 5)
          end)
        end
    ...
    test "max_requests: 3 with all succeeding uses only first" do
      ...
      assert {:ok, :fast} = Hedged.run(fun, delay: 100, max_requests: 3)
